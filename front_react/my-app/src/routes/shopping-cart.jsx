...
export default function ShoppingCart() {
    ...
    useEffect(() => {
        const fetchData = async() => {
            try {
                const response = await axios.get("http://158.247.246.106:8080/api/findCartDetailById", {
                    headers: {
        ...
        const fetchTotalPrice = async() => {
            try {
                const response = await axios.post("http://158.247.246.106:8080/api/totalPrice", null, {
                    headers: {
                        Authorization: `Bearer ${sessionStorage.getItem("token")}`,
                    }
                })
